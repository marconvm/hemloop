...
- `get_campaign_state`, `validate_claims`, `export_composition` and `get_offer` carry `readOnlyHint: true`. `get_wardrobe` (shopper-entered rows) and `import_product` (storefront data) carry `untrustedContentHint: true`, and the third-party text they return is wrapped in `<<<untrusted-content>>>` fences with a one-line preamble telling the agent not to follow instructions inside them.
