...
**As a merchant** (`/studio`): human-approved requests arrive in the studio's **Incoming requests** panel without a shopper identifier or wardrobe rows. Requests are grouped by category and size, with a count ("hoodie · M · 4 requests"), and each is labelled **Need** or **Want** so you can see which is a gap in someone's wardrobe and which is a stated preference. Each group also carries a verdict against the stock you locked: **can offer**, **size out of stock**, or **other category**, with the reason on hover, so you can see at a glance which demand you simply cannot fill today. Where a group says "replacing one they own", those shoppers already have the item and it is worn out. Your agent reads the same rows with `get_demand`. You answer one with a campaign: press **Unlock offer facts**, edit them, press **Answer this request** to pull the product in, then **Lock offer facts** before your agent produces. Rendered claims that contradict the locked facts are rejected before they apply. Placement (Story 9:16, Feed 4:5, Display 16:9) is a human-only choice above the canvas. The promo video below is one output of that workflow. Below the facts sit the **offer rules** (cost price, margin floor, max discount) and, on each incoming request, a **Propose offer** button your agent's `propose_offer` call can also trigger, plus an **Auto-propose** toggle; see "Personal offers" below.
