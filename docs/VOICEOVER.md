...
Now watch. The agent tries to share that demand — and the page refuses. Sharing needs a human. One click arms a single approval, the retry goes through, and the tool shows the entire payload it sent: a category, a size, a product. No name. No I-D. No wardrobe. And the approval? Already spent.

## VO-04 demand lands (1:00)

On the merchant side, that event just arrived — demand data stores have never had, minimized at the source. The merchant pulls the product in and locks the campaign truth. Locking is a button. Deliberately not a tool.
...
Shoppers keep their data. Merchants finally see demand. And everything produced in response is provably true. Hemloop — built on WebMCP.
