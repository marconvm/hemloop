...
| 2:35-2:40 | Close on the closed loop | The rail, all seven stations lit, Loop Room (or both surfaces) side by side. Repo + docs flash | "One request. Private closet, real demand, an offer that cannot lie, and a sale the merchant can attribute. Hemloop, built on WebMCP." |
...
|---|---|---|---|---|
| C1 | 0:00 | closet | `What should I buy next?` | `find_gaps` returns three rows: no hoodie, only one jacket, footwear due at 21 months |
...
| C3 | 0:20 | closet | `Yes, send it.` | succeeds; the tool returns the exact payload sent, with no identity field |
...
**Receipt import (preferred first move on the Loop Room):** upload one of the sample images in `public/receipts/` in the ChatGPT chat; the agent reads it and calls `import_receipt` with the text. That is station **New item**. On `/closet`, paste a sample order email instead.
