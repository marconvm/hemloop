...
- Live headers on `/`: HSTS, Permissions-Policy, Referrer-Policy, X-Content-Type-Options,
  ...
  > **CORRECTION (added after review, this brief was wrong).** `facts.sh` probes only `/`, and `/` is
  > a dynamic RSC render, so the absence of `cache-control` there is correct, not a gap. Measured on
  > the real asset paths: `/_next/static/chunks/*.js` and `/_next/static/css/*.css` both return
  > `public, max-age=31536000, immutable` with `cf-cache-status: HIT` (from `dist/client/_headers`,
  > generated by vinext), and un-hashed `public/*` files return Workers Static Assets' default
  > `public, max-age=0, must-revalidate` with an etag. Caching was already right. Reviewer 2 caught
  > this; the lesson is that a one-route fact sheet is not a fact sheet.
