...
- **`docs/coordination/`**: CODEX-COORDINATION.md, the judge reviews and the surface snapshots are in the public repo. They are honest but they are working notes, and one of them narrates a bug hunt in the tool count. Not shipped to `/docs/`, but a judge who clones will find them. Consider moving to a branch.
